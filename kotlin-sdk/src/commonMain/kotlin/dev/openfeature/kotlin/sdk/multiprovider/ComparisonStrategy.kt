...
/**
 * A [MultiProvider.Strategy] that evaluates all providers and compares their results.
 *
 * This strategy is useful for migration validation — it evaluates every provider, compares their
 * returned values, and reports mismatches via the [onMismatch] callback.
 *
 * @param fallbackProvider The provider whose result is used when providers disagree.
 *   Must be the same object reference as one of the providers in the [MultiProvider] constructor.
 * @param onMismatch Optional callback invoked when providers return different values.
 */
class ComparisonStrategy(
    private val fallbackProvider: FeatureProvider,
    ...
    private fun <T> resolveFallbackEvaluation(
        ...
        return allEvaluations.firstOrNull { (provider, _) ->
            unwrapProvider(provider) === unwrapProvider(fallbackProvider)
        } ?: throw OpenFeatureError.GeneralError(
            "Fallback provider not found in resolution results"
        )
    }
